...
import {
  aboutPageMeta as staticPageMeta,
  aboutStory as staticStory,
  aboutVision as staticVision,
  aboutMission as staticMission,
  aboutPillars as staticPillars,
  values as staticValues,
  teamMembers as staticTeam,
} from "@/content";
import type { LucideIcon } from "lucide-react";
import type { AboutData, StoryData } from "@/sanity/lib/queries";

const ICON_MAP: Record<string, LucideIcon> = {
  Heart, Eye, Users, Award, Shield, GraduationCap, Trophy, Target,
};
...
export default function AboutPageContent({ aboutData, storiesData }: AboutPageContentProps) {
  const [activeMissionIndex, setActiveMissionIndex] = useState(0);

  const pageMeta = aboutData ? {
    title: aboutData.pageTitle ?? staticPageMeta.title,
    breadcrumb: aboutData.pageBreadcrumb ?? staticPageMeta.breadcrumb,
    tagline: aboutData.pageTagline ?? staticPageMeta.tagline,
  } : staticPageMeta;

  const story = aboutData ? {
    paragraphs: aboutData.storyParagraphs?.length ? aboutData.storyParagraphs : staticStory.paragraphs,
    foundingDate: aboutData.foundingDate ?? staticStory.foundingDate,
  } : staticStory;

  const visionText = aboutData?.visionText ?? staticVision.text;

  const missionPoints = aboutData?.missionPoints?.length ? aboutData.missionPoints : staticMission.points;

  const pillars = aboutData?.pillars?.length ? aboutData.pillars : staticPillars;

  const valuesList = aboutData?.values?.length ? aboutData.values : staticValues;

  const team = aboutData?.teamMembers?.length ? aboutData.teamMembers : staticTeam;
  ...
  return (
    <>
        ...
        <Container size="lg">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="space-y-6">
              <SectionHeading label="Who We Are" title="Our Story" align="left" className="mb-0" />
              {story.paragraphs.map((p, i) => (
                <p key={i} className="text-text-muted leading-relaxed">{p}</p>
              ))}
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/5 border border-primary/10">
                <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                <span className="text-sm font-semibold text-primary">Founded {story.foundingDate}</span>
              </div>
            </div>
            <div className="space-y-4">
              <ImagePlaceholder src="/images/about/story.svg" alt="Shiv Prabha Foundation community event" aspect="4/3" />
              <ImagePlaceholder src="/images/about/founding.svg" alt="Founding day — 13th February 2025" aspect="16/9" />
            </div>
          </div>
        </Container>
      </Section>
        ...
        <Container size="lg" className="relative z-10">
              ...
              {missionPoints.map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-5 text-left rounded-[var(--radius-md)] border transition-all duration-300 group ${isActive ? "bg-surface border-primary shadow-soft translate-x-1" : "bg-surface/50 border-border-light hover:bg-surface hover:border-primary/30"}`}
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${isActive ? "bg-primary text-white border-primary" : "bg-primary/5 text-primary border-primary/10 group-hover:bg-primary/10"}`}>
                      <IconComponent size={18} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className={`text-[9px] uppercase tracking-[0.15em] block font-extrabold ${isActive ? "text-primary" : "text-text-muted/60"}`}>
                        {meta.philosophy}
                      </span>
                      <span className={`font-bold text-base transition-colors duration-200 block truncate ${isActive ? "text-text-dark" : "text-text-muted group-hover:text-text-dark"}`}>
                        {meta.title}
                      </span>
                    </div>
                    <ArrowRight size={16} className={`transition-all duration-300 ${isActive ? "text-primary opacity-100 translate-x-0" : "text-text-muted/0 opacity-0 -translate-x-2 group-hover:text-primary group-hover:opacity-100 group-hover:translate-x-0"}`} />
                  </button>
                );
              })}
            </div>
                ...
                return (
                  <div className="bg-surface border border-border-light rounded-[var(--radius-lg)] p-5 sm:p-8 md:p-10 shadow-soft h-full flex flex-col justify-between min-h-[460px] relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <h4 className="text-xs font-bold text-primary uppercase tracking-widest mb-1.5">Our Objective</h4>
                          <p className="text-lg text-text-dark leading-relaxed font-medium">
                            {missionPoints[detailIdx]}
                          </p>
                        </div>
                        <div>
                          <h4 className="text-xs font-bold text-text-muted uppercase tracking-widest mb-1.5">Action Plan &amp; Strategy</h4>
                    ...
                    <div className="mt-8 pt-6 border-t border-border-light/60 flex flex-wrap gap-4 items-center justify-between">
                      ...
                      <Link href={MISSION_STRATEGIES[detailIdx]?.link || "#"} className="inline-flex items-center gap-1.5 text-xs md:text-sm font-extrabold text-primary hover:text-text-dark transition-colors duration-200">
          ...
          <div className="lg:hidden space-y-4">
            {missionPoints.map((point, i) => {
              ...
              return (
                <div key={i} className={`border rounded-[var(--radius-md)] overflow-hidden transition-all duration-300 bg-surface ${isOpen ? "border-primary shadow-soft" : "border-border-light"}`}>
                  <button
                    onClick={() => setActiveMissionIndex(isOpen ? -1 : i)}
                    className="w-full flex items-center justify-between p-5 text-left font-heading"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-colors ${isOpen ? "bg-primary text-white border-primary" : "bg-primary/5 text-primary border-primary/10"}`}>
                  ...
                  <div className={`transition-all duration-300 ease-in-out ${isOpen ? "max-h-[500px] border-t border-border-light/60 p-5" : "max-h-0 overflow-hidden"}`}>
                    <div className="space-y-4 text-xs md:text-sm">
                      <div>
                        <h4 className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Our Objective</h4>
                        <p className="text-text-dark leading-relaxed font-medium">{point}</p>
                      </div>
                      <div>
                        <h4 className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">Action Plan &amp; Strategy</h4>
                      ...
                      <div className="pt-2">
                        <Link href={meta.link} className="inline-flex items-center gap-1.5 text-xs font-bold text-primary hover:text-primary-dark">
      ...
      <Section spacing="lg" background="white">
        <Container size="lg">
          <SectionHeading label="What We Stand For" title="Our Core Pillars" description="Three areas that define our work and our commitment to society." />
      ...
      <Section spacing="lg" background="default">
        <Container size="lg">
          <SectionHeading label="What Guides Us" title="Our Core Values" description="Every decision we make is rooted in these principles." />
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {valuesList.map((v) => {
              const Icon = ICON_MAP[v.iconName] || Heart;
              return (
                <div key={v.title} className="bg-surface border border-border-light/60 rounded-[var(--radius-md)] p-5 sm:p-6 md:p-8 text-center shadow-soft hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      ...
      <Section spacing="lg" background="white">
        <Container size="lg">
          <SectionHeading label="Our People" title="Leadership Team" description="Dedicated individuals driving our mission forward." />
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {team.map((member) => (
              <div key={member.name} className="bg-surface border border-border-light/60 rounded-[var(--radius-md)] p-5 sm:p-6 md:p-8 text-center shadow-soft hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                {member.image ? (
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden mx-auto mb-5 relative border-2 border-primary/20">
                    <Image src={(member.image as { asset?: { url: string } })?.asset?.url ?? "/images/placeholder.svg"} alt={member.name} fill className="object-cover" />
                  </div>
                ) : (
        ...
        <Container size="md">
          <div className="text-center space-y-8">
            ...
            <p className="text-surface/80 text-lg max-w-xl mx-auto font-medium">
              Be part of a brighter tomorrow. Support Shiv Prabha Foundation and help us create lasting change for those who need it most.
            </p>

...
import {
  volunteerPageMeta as staticPageMeta,
  volunteerRoles as staticRoles,
  volunteerBenefits as staticBenefits,
  volunteerCommunityStats as staticCommunityStats,
  volunteerFormOptions as staticFormOptions,
} from "@/content";
import { submitVolunteerForm } from "@/app/actions/volunteer";
import type { LucideIcon } from "lucide-react";
import type { VolunteerPageData } from "@/sanity/lib/queries";
...
export default function VolunteerPageContent({ data }: VolunteerPageContentProps) {
  const [submitted, setSubmitted] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const pageMeta = {
    title: data?.pageTitle ?? staticPageMeta.title,
    subtitle: data?.pageSubtitle ?? staticPageMeta.subtitle,
    breadcrumb: data?.pageBreadcrumb ?? staticPageMeta.breadcrumb,
  };

  const roles = data?.roles?.length ? data.roles : staticRoles;
  const benefits = data?.benefits?.length ? data.benefits : staticBenefits;
  const communityStats = {
    count: data?.communityCount ?? staticCommunityStats.count,
    label: data?.communityLabel ?? staticCommunityStats.label,
    description: data?.communityDescription ?? staticCommunityStats.description,
  };
  const formOptions = {
    interests: data?.formInterestOptions?.length ? data.formInterestOptions : staticFormOptions.interests,
    availability: data?.formAvailabilityOptions?.length ? data.formAvailabilityOptions : staticFormOptions.availability,
  };
              ...
              <div className="bg-white border border-border-light/60 rounded-2xl p-5 sm:p-6 md:p-8 shadow-sm flex items-start gap-5 mt-8">
                ...
                <div>
                  <h3 className="font-extrabold font-heading text-xl sm:text-2xl text-secondary mb-1">
                    {communityStats.count} {communityStats.label}
                  </h3>
